...
/**
 * SwapModal - Binance Premium Design
 */
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0, 0, 0, 0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: '20px', fontFamily: 'Inter, sans-serif' }}>
      <div style={{ background: 'rgba(18, 22, 28, 0.98)', backdropFilter: 'blur(12px)', border: '1px solid #1E2329', borderRadius: '14px', padding: '32px', maxWidth: '500px', width: '100%', position: 'relative', boxShadow: '0 10px 30px rgba(0, 0, 0, 0.6)' }}>
        ...
        <h2 style={{ fontSize: '24px', fontWeight: '700', color: '#EAECEF', margin: '0 0 24px 0', display: 'flex', alignItems: 'center', gap: '12px' }}>
          <IoSwapHorizontalOutline size={32} color="#7B2CFF" />
          Swap Crypto
        </h2>

        <div style={{ marginBottom: '16px', background: '#0B0E11', borderRadius: '12px', padding: '16px', border: '1px solid #1E2329' }}>
          <div style={{ fontSize: '13px', color: '#B7BDC6', marginBottom: '8px', fontWeight: '500' }}>From</div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <select value={from} onChange={(e) => setFrom(e.target.value)} style={{ padding: '12px', background: '#12161C', border: '1px solid #1E2329', borderRadius: '12px', color: '#EAECEF', fontSize: '16px', fontWeight: '600', cursor: 'pointer', minWidth: '120px' }}>
              {availableCoins.map(coin => <option key={coin} value={coin}>{coin}</option>)}
            </select>
            <input type="number" value={fromAmount} onChange={(e) => setFromAmount(e.target.value)} placeholder="0.00" style={{ flex: 1, padding: '12px', background: '#12161C', border: '1px solid #1E2329', borderRadius: '12px', color: '#EAECEF', fontSize: '16px', fontWeight: '600', outline: 'none' }} />
          </div>
          <div style={{ fontSize: '13px', color: '#B7BDC6', marginTop: '8px' }}>Available: {fromBalance.toFixed(8)} {from}</div>
        </div>
        ...
        <div style={{ marginBottom: '20px', background: '#0B0E11', borderRadius: '12px', padding: '16px', border: '1px solid #1E2329' }}>
          <div style={{ fontSize: '13px', color: '#B7BDC6', marginBottom: '8px', fontWeight: '500' }}>To</div>
          <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
            <select value={to} onChange={(e) => setTo(e.target.value)} style={{ padding: '12px', background: '#12161C', border: '1px solid #1E2329', borderRadius: '12px', color: '#EAECEF', fontSize: '16px', fontWeight: '600', cursor: 'pointer', minWidth: '120px' }}>
              {balances.map(b => <option key={b.currency} value={b.currency}>{b.currency}</option>)}
            </select>
            <input type="text" value={toAmount} readOnly placeholder="0.00" style={{ flex: 1, padding: '12px', background: '#12161C', border: '1px solid #1E2329', borderRadius: '12px', color: '#EAECEF', fontSize: '16px', fontWeight: '600' }} />
          </div>
        </div>
        ...
        {rate > 0 && (
          <div style={{ background: 'rgba(123, 44, 255, 0.1)', border: '1px solid rgba(123, 44, 255, 0.3)', borderRadius: '12px', padding: '12px', marginBottom: '20px' }}>
            <div style={{ fontSize: '13px', color: '#B7BDC6' }}>Exchange Rate</div>
            <div style={{ fontSize: '16px', fontWeight: '600', color: '#7B2CFF', marginTop: '4px' }}>1 {from} = {rate.toFixed(6)} {to}</div>
          </div>
        )}

        <button onClick={handleSwap} disabled={isSubmitting || !fromAmount || parseFloat(fromAmount) > fromBalance} style={{ width: '100%', padding: '14px', background: isSubmitting || !fromAmount ? '#6B7280' : '#7B2CFF', border: 'none', borderRadius: '12px', color: '#FFFFFF', fontSize: '16px', fontWeight: '600', cursor: isSubmitting || !fromAmount ? 'not-allowed' : 'pointer' }}>
