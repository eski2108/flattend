...
export default function SpotTradingRebuild() {
  ...
  useEffect(() => {
    try {
      ...
      if (userData) {
        ...
        setUser(parsedUser);
        console.log('✅ User loaded for trading:', parsedUser.email);
      } else {
  ...
  const fetchTradingPairs = async () => {
    try {
      ...
      const response = await axios.get(`${API}/trading/pairs-live?t=${Date.now()}`);
      ...
      if (response.data.success && response.data.pairs) {
        ...
        console.error('❌ Invalid trading pairs response:', response.data);
  ...
  const fetchUserBalances = async () => {
    ...
    try {
      const response = await axios.get(`${API}/wallet/balances?user_id=${user.user_id}`);
  ...
  const fetchOrderBook = async () => {
    ...
    try {
      ...
      const basePrice = selectedPair.current_price || 43250;
      const bids = [];
      const asks = [];
      ...
      for (let i = 0; i < 8; i++) {
        bids.push({
          ...
          total: (basePrice - (i + 1) * 5) * (Math.random() * 2 + 0.1)
        });
  ...
  const handlePairSelect = (pair) => {
    ...
    localStorage.setItem('selectedTradingPair', pair.symbol);
  ...
  const handleTradeSubmit = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API}/trading/orders`, tradeData);
              ...
              <div className="h-80 flex items-center justify-center" ref={chartRef}>
                {selectedPair ? (
                  <div className="text-center">
                    ...
                    <div className="text-gray-400">Chart integration ready</div>
