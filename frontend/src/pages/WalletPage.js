...
// MASSIVE emoji map - covers ALL 238+ coins
...
// CoinIcon component with proper React state-based fallback
const CoinIcon = ({ symbol, logoUrl, size = 40 }) => {
  const [imgError, setImgError] = useState(false);
  const emoji = getCoinEmoji(symbol);
  
  // If img failed to load, show emoji
  if (imgError) {
    return (
      <div style={{
        width: `${size}px`,
...
export default function WalletPage() {
  ...
  // Transactions now loaded on separate /transactions page
