        ...
        {activeTab === 'Activity' && (
          <div style={{ padding: '0 20px' }}>
            {transactions.length === 0 ? (
              <div style={{ padding: '60px 20px', textAlign: 'center', color: '#8FA3C8' }}>
                No transactions yet
              </div>
            ) : (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {transactions.map((tx, idx) => (
                  ...
                  >
                    <div>
                      <div style={{ fontSize: '14px', fontWeight: '500', color: '#fff', textTransform: 'capitalize', marginBottom: '4px' }}>
                        {tx.type || 'Transaction'}
                      </div>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: '14px', fontWeight: '600', color: tx.type === 'deposit' ? '#16C784' : '#fff' }}>
                        {tx.type === 'deposit' ? '+' : '-'}{tx.amount} {tx.currency}
                      </div>
                      <div style={{ fontSize: '11px', color: tx.status === 'completed' ? '#16C784' : '#F0B90B', textTransform: 'uppercase', marginTop: '2px' }}>
                        {tx.status || 'Unknown'}
                      </div>
                    </div>
                  </div>
        ...
        {activeTab === 'Portfolio' && (
          <div style={{ padding: '0 20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              {balances.filter(b => b.total_balance > 0).sort((a, b) => (b.gbp_value || 0) - (a.gbp_value || 0)).map(balance => {
                const coinMeta = allCoins.find(c => c.symbol === balance.currency);
                const totalValue = balances.reduce((sum, b) => sum + (b.gbp_value || 0), 0);
                const allocation = totalValue > 0 ? ((balance.gbp_value || 0) / totalValue * 100).toFixed(1) : 0;
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <img src={getCoinLogo(balance.currency)} alt={balance.currency} style={{ width: '36px', height: '36px', borderRadius: '50%' }} />
                        <div>
                          <div style={{ fontSize: '15px', fontWeight: '600', color: '#fff' }}>{balance.currency}</div>
                          <div style={{ fontSize: '12px', color: '#6B7A99' }}>{coinMeta?.name || balance.currency}</div>
                        </div>
                      </div>
                      <div style={{ textAlign: 'right' }}>
                        <div style={{ fontSize: '16px', fontWeight: '600', color: '#00E5FF' }}>
                          £{(balance.gbp_value || 0).toFixed(2)}
                        </div>
                        <div style={{ fontSize: '12px', color: '#6B7A99' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '8px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
                      <div style={{ fontSize: '12px', color: '#6B7A99' }}>Allocation: <span style={{ color: '#fff', fontWeight: '500' }}>{allocation}%</span></div>
                    </div>
                  </div>
                );
              })}
