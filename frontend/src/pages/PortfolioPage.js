        ...
        <div style={{
          ...
          {portfolio.length > 0 ? (
              ...
              <RechartsPieChart>
                <Pie
                  data={portfolio.map(item => ({
                    name: item.currency,
                    value: item.current_value_usd || 0
                  }))}
                  cx="50%"
                  cy="50%"
                  labelLine={false}
                ...
                >
                  {portfolio.map((entry, index) => {
                    const colors = ['#00F6FF', '#00E0FF', '#22C55E', '#FFD700', '#FF6B6B', '#9D4EDD', '#06FFA5', '#FF0080', '#00D9FF', '#FFB800'];
                    return <Cell key={`cell-${index}`} fill={colors[index % colors.length]} />;
                  })}
                </Pie>
                <Tooltip 
                  formatter={(value) => `$${Number(value).toFixed(2)}`}
                  contentStyle={{ background: '#1A1D26', border: '1px solid #00E0FF', borderRadius: '8px' }}
            ...
            <div style={{ 
              ...
              <div style={{ color: '#A8A8A8', fontSize: '1.1rem', textAlign: 'center' }}>
                Your portfolio allocation will appear here once you hold assets
              </div>
              <div style={{ color: '#00E0FF', fontSize: '0.9rem' }}>
                Start trading to see your portfolio breakdown
              </div>
            </div>
          )}
        </div>
