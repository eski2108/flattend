...
export default function InstantSellNew() {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        response.data.balances?.forEach(b => {
          if (b.currency !== 'GBP') {
            balances[b.currency] = b.available_balance || 0;
          }
        });
  ...
  const generateQuote = async () => {
    ...
    const balance = cryptoBalances[selectedCrypto] || 0;
  ...
  const currentBalance = cryptoBalances[selectedCrypto] || 0;
        ...
        <div className="max-w-2xl mx-auto">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-400">
              Get GBP instantly at locked prices
            </p>
          </div>
          ...
          <div className="bg-gray-800/50 backdrop-blur-lg rounded-2xl border border-gray-700 p-8">
            ...
            {step === 'input' && (
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">
                    Amount of {selectedCoin?.name} to Sell
                  </label>
                  <input
                    type="number"
                    value={amount}
                    ...
                    placeholder="0.01"
                    step="0.001"
                    className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:border-orange-500"
                  />
                </div>

                <div className="flex items-center gap-2 p-4 bg-orange-500/10 border border-orange-500/30 rounded-xl">
                  <IoWallet className="text-orange-400 text-xl" />
                  <div>
                    <div className="text-xs text-orange-300">Available {selectedCrypto}</div>
                    <div className="text-lg font-bold text-orange-200">{currentBalance.toFixed(8)}</div>
                  </div>
                </div>

                <button
                  onClick={generateQuote}
                  disabled={loading || !amount || currentBalance === 0}
                  className="w-full py-4 bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 disabled:from-gray-600 disabled:to-gray-700 text-white font-bold rounded-xl transition-all disabled:cursor-not-allowed"
                >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-br from-orange-500/20 to-red-500/20 border border-orange-500/30 rounded-xl p-6">
                  <div className="text-center mb-4">
                    <div className="text-gray-400 text-sm mb-1">You're selling</div>
                  ...
                  <div className="grid grid-cols-2 gap-4 mt-6">
                    ...
                    <div className="bg-orange-500/20 rounded-lg p-3">
                      <div className="text-xs text-orange-300 mb-1">Your Locked Price</div>
                  ...
                  <div className="mt-4 p-3 bg-gray-800/50 rounded-lg">
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-gray-400">Gross Payout</span>
                      <span className="text-white">£{quote.gross_payout.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-gray-400">Platform Fee ({quote.fee_percent}%)</span>
                      <span className="text-red-400">-£{quote.fee_amount.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-gray-400">Spread</span>
                      <span className="text-red-400">{quote.spread_percent}%</span>
                    </div>
                    <div className="border-t border-gray-700 mt-2 pt-2">
                      <div className="flex justify-between font-bold">
                        <span className="text-white">You'll Receive</span>
                        <span className="text-xl text-orange-400">£{quote.net_payout.toFixed(2)}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-xl">
                  ...
                  <div className="text-sm text-yellow-200">
                    <strong>Price Locked!</strong> This payout is guaranteed for 5 minutes. 
                    The market price may change, but you'll receive exactly £{quote.net_payout.toFixed(2)}.
                  </div>
                </div>
            ...
            {step === 'executing' && (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">Processing Your Sale...</h3>
            ...
            {step === 'success' && quote && (
              <div className="text-center py-12">
                ...
                <p className="text-gray-400 mb-4">GBP has been added to your wallet</p>
                <div className="bg-gray-700/50 rounded-xl p-4 inline-block">
                  <div className="text-3xl font-bold text-green-400 mb-1">
                    +£{quote.net_payout.toFixed(2)}
                  </div>
                  <div className="text-sm text-gray-400">
                    for {quote.crypto_amount} {quote.crypto_currency}
                  </div>
                </div>
              </div>
            )}
