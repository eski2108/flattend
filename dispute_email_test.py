        ...
        if success:
            ...
            if success:
                try:
                    ...
                    self.log_test("Buyer Login", False, "Invalid JSON response")
                    return False
            else:
        ...
        if success:
            try:
                ...
                if data.get('success') and data.get('user'):
                    ...
                    self.log_test("Seller Registration", False, "No user data in response")
                    return False
            except:
                self.log_test("Seller Registration", False, "Invalid JSON response")
                return False
        else:
            # Try login if user already exists
            success, response = self.make_request('POST', '/auth/login', data={
