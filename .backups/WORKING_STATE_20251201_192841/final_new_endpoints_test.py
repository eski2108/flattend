...
class ComprehensiveNewEndpointsTester:
    ...
    def setup_test_user(self):
        """Create a test user for endpoints that require valid user_id"""
        ...
        result = self.make_request("POST", "/auth/register", test_user_data)
        ...
        if response.status_code == 200:
            try:
                ...
                if data.get("success") and data.get("user", {}).get("user_id"):
                    ...
                    print(f"❌ User registration response missing user_id: {data}")
            ...
            if len(result) == 3:  # Error case
                response, response_time, error = result
                print(f"❌ User login failed: {error}")
                return False
    ...
    def test_p2p_notifications_get(self):
        ...
        endpoint = f"/p2p/notifications/{user_id}"
    ...
    def test_wallet_credit(self):
        ...
        data = {
            "user_id": self.test_user_id or "test",
            "currency": "BTC",
            "amount": 1.0,
    ...
    def run_all_tests(self):
        ...
        print("🎯 COMPREHENSIVE BACKEND API ENDPOINT VERIFICATION - ALL NEW FEATURES")
        print("=" * 80)
        ...
        if not self.setup_test_user():
            print("⚠️ Warning: Could not create test user. Some tests may fail.")
    ...
    def print_final_summary(self):
        ...
        for result in self.test_results:
            ...
            print(f"{status} {endpoint}: {message}{time_info}")
        
        print("\n🔍 CRITICAL VERIFICATION RESULTS:")
        
        # Check critical requirements
        all_endpoints_accessible = self.passed_tests == self.total_tests
        no_500_errors = all("500" not in str(r.get("details", "")) for r in self.test_results)
...
def main():
    ...
    tester = ComprehensiveNewEndpointsTester()
