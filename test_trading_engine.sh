...
curl -s "https://trading-perf-boost.preview.emergentagent.com/api/prices/live" | python3 -c "import sys, json; data=json.load(sys.stdin); print('✅ Price feed working') if data.get('success') else print('❌ Price feed failed')"

echo ""
echo "[3] Testing market-price endpoint..."
curl -s "https://trading-perf-boost.preview.emergentagent.com/api/market-price?pair=BTCUSD" | python3 -c "import sys, json; data=json.load(sys.stdin); print('✅ Market price: $' + str(data['price'])) if data.get('success') else print('❌ Market price failed')"
