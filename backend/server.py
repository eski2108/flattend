...
PLATFORM_CONFIG = {
    ...
    "withdraw_fee_percent": 1.0,  # 1% withdrawal fee
    "p2p_trade_fee_percent": 1.0,  # 1% P2P trade fee
    "spot_trading_fee_percent": 1.0,
    "swap_fee_percent": 3.0,  # 3% Swap fee
    "instant_buy_fee_percent": 3.0,  # 3% Instant Buy fee
    "express_buy_fee_percent": 2.0,  # 2% Express Buy (P2P Express) fee
    "paypal_to_paypal_fee_percent": 3.0,  # 3% PayPal to PayPal fee
    "admin_liquidity_buy_spread_percent": 1.0,  # 1% buy spread
    "admin_liquidity_sell_spread_percent": 1.0,  # 1% sell spread
    "savings_early_withdrawal_penalty_percent": 2.0,  # 2% early withdrawal penalty
    "staking_admin_fee_percent": 1.0,  # 1% staking fee
...
async def get_user_seller_link(user_id: str):
    ...
    try:
        ...
        base_url = os.environ.get("REACT_APP_BACKEND_URL", "https://coinhubx-refresh.preview.emergentagent.com")
...
async def google_auth():
    ...
    try:
        ...
        redirect_uri = "https://coinhubx-refresh.preview.emergentagent.com/api/auth/google/callback"
...
async def google_callback(code: str = None, error: str = None):
    ...
    frontend_url = os.environ.get('FRONTEND_URL', 'https://coinhubx-refresh.preview.emergentagent.com')
...
async def complete_google_signup(request: dict):
    ...
    <head>
        <title>Email Verified - Coin Hub X</title>
        <meta http-equiv="refresh" content="3;url=https://coinhubx-refresh.preview.emergentagent.com/login">
        <style>
            body { 
                font-family: Arial, sans-serif; 
                background: linear-gradient(135deg, #000D1A, #1a1f3a);
    ...
    <body>
        <div class="container">
            ...
            <a href="https://coinhubx-refresh.preview.emergentagent.com/login">Go to Login</a>
        </div>
    </body>
    </html>
    """
...
async def forgot_password(request: ForgotPasswordRequest, req: Request):
    ...
    try:
        ...
        reset_link = f"https://coinhubx-refresh.preview.emergentagent.com/reset-password?token={reset_token}"
                    ...
                    html_content=f'''
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                            <div style="background: linear-gradient(135deg, #000D1A, #1a1f3a); padding: 40px; text-align: center; border-radius: 10px;">
                                ...
                                <a href="https://coinhubx-refresh.preview.emergentagent.com/dashboard" style="display: inline-block; background: linear-gradient(135deg, #00F0FF, #A855F7); color: #000; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 30px;">
                                    View on Platform
                                </a>
                            </div>
                        </div>
...
async def initiate_withdrawal(request: InitiateWithdrawalRequest, req: Request):
    ...
    if security_level == "admin_approval_required":
        ...
        try:
            ...
            confirmation_url = f"{os.environ.get('FRONTEND_URL', 'https://coinhubx-refresh.preview.emergentagent.com')}/confirm-withdrawal?token={confirmation_token}"
...
async def get_complete_revenue(period: str = "all"):
    ...
    try:
        ...
        for txn in all_txns:
            ...
            if "p2p" in txn_type.lower() or "p2p" in source.lower():
                breakdown["p2p"] += fee
            elif "swap" in txn_type.lower() or "swap" in source.lower():
                breakdown["swap"] += fee
            elif "instant_buy" in source.lower() or "instant-buy" in source.lower():
                breakdown["instantBuy"] += fee
            elif "instant_sell" in source.lower() or "instant-sell" in source.lower():
                breakdown["instantSell"] += fee
            elif "express" in txn_type.lower() or "express" in source.lower():
                breakdown["expressBuy"] += fee
            elif "withdraw" in txn_type.lower():
                breakdown["withdrawals"] += fee
            elif "deposit" in txn_type.lower():
                breakdown["deposits"] += fee
            elif "paypal" in source.lower():
                breakdown["paypal"] += fee
            elif "liquidity" in source.lower():
                breakdown["liquiditySpread"] += fee
            elif "early" in source.lower() or "penalty" in source.lower():
                breakdown["earlyWithdrawal"] += fee
            elif "staking" in source.lower() or "stake" in source.lower():
                breakdown["staking"] += fee
            elif "cross" in source.lower() or "conversion" in source.lower():
                breakdown["crossWallet"] += fee
            elif "internal" in source.lower() or "transfer" in source.lower():
                breakdown["internalTransfer"] += fee
        
        total = sum(breakdown.values())
        
        # Calculate period-specific totals
        today_txns = [t for t in all_txns if datetime.fromisoformat(t["created_at"]) >= (now - timedelta(days=1))]
        week_txns = [t for t in all_txns if datetime.fromisoformat(t["created_at"]) >= (now - timedelta(days=7))]
        month_txns = [t for t in all_txns if datetime.fromisoformat(t["created_at"]) >= (now - timedelta(days=30))]
        
        today_total = sum(t.get("fee", 0) for t in today_txns)
        week_total = sum(t.get("fee", 0) for t in week_txns)
        month_total = sum(t.get("fee", 0) for t in month_txns)
