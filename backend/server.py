...
async def get_user_seller_link(user_id: str):
    ...
    try:
        ...
        base_url = os.environ.get("REACT_APP_BACKEND_URL", "https://tradingplatform-14.preview.emergentagent.com")
...
async def google_auth():
    ...
    try:
        ...
        redirect_uri = "https://tradingplatform-14.preview.emergentagent.com/api/auth/google/callback"
...
async def google_callback(code: str = None, error: str = None):
    ...
    frontend_url = os.environ.get('FRONTEND_URL', 'https://tradingplatform-14.preview.emergentagent.com')
...
async def complete_google_signup(request: dict):
    ...
    <head>
        <title>Email Verified - Coin Hub X</title>
        <meta http-equiv="refresh" content="3;url=https://tradingplatform-14.preview.emergentagent.com/login">
        <style>
            body { 
                font-family: Arial, sans-serif; 
                background: linear-gradient(135deg, #000D1A, #1a1f3a);
    ...
    <body>
        <div class="container">
            ...
            <a href="https://tradingplatform-14.preview.emergentagent.com/login">Go to Login</a>
        </div>
    </body>
    </html>
    """
...
async def forgot_password(request: ForgotPasswordRequest, req: Request):
    ...
    try:
        ...
        reset_link = f"https://tradingplatform-14.preview.emergentagent.com/reset-password?token={reset_token}"
                    ...
                    html_content=f'''
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                            <div style="background: linear-gradient(135deg, #000D1A, #1a1f3a); padding: 40px; text-align: center; border-radius: 10px;">
                                ...
                                <a href="https://tradingplatform-14.preview.emergentagent.com/dashboard" style="display: inline-block; background: linear-gradient(135deg, #00F0FF, #A855F7); color: #000; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 30px;">
                                    View on Platform
                                </a>
                            </div>
                        </div>
...
async def initiate_withdrawal(request: InitiateWithdrawalRequest, req: Request):
    ...
    if security_level == "admin_approval_required":
        ...
        try:
            ...
            confirmation_url = f"{os.environ.get('FRONTEND_URL', 'https://tradingplatform-14.preview.emergentagent.com')}/confirm-withdrawal?token={confirmation_token}"
...
async def get_user_crypto_transactions(user_id: str, limit: int = 100):
    ...
    try:
        ...
        # Convert all datetime objects to ISO strings first for consistent sorting
        from datetime import datetime
        for tx in all_transactions:
            created_at = tx.get("created_at")
            if isinstance(created_at, datetime):
                tx["created_at"] = created_at.isoformat()
            elif created_at is None or created_at == "":
                tx["created_at"] = "1970-01-01T00:00:00"
        
        # Sort all transactions by date string (newest first)
        all_transactions.sort(key=lambda x: x.get("created_at", "1970-01-01T00:00:00"), reverse=True)
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "referral_link": f"https://tradingplatform-14.preview.emergentagent.com/register?ref={user.get('referral_code', user_id[:8].upper())}",
...
async def get_all_admin_liquidity():
    ...
    try:
        ...
        logger.error(f"Error fetching liquidity: {str(e)}")
        return {
            "success": False,
            "error": str(e)
        }
...
async def update_admin_liquidity(request: dict):
    ...
    try:
        ...
        if not currency or new_balance < 0:
            return {
                "success": False,
                "message": "Invalid currency or balance"
            }
        ...
        logger.info(f"Admin liquidity updated: {currency} = {new_balance}")
        
        return {
            "success": True,
            "message": f"{currency} balance updated to {new_balance}"
        }
    except Exception as e:
        logger.error(f"Error updating liquidity: {str(e)}")
        return {
            "success": False,
            "message": str(e)
        }
