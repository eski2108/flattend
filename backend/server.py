...
async def get_user_seller_link(user_id: str):
    ...
    try:
        ...
        base_url = os.environ.get("REACT_APP_BACKEND_URL", "https://coinhubx-ui-refresh.preview.emergentagent.com")
...
async def google_callback(code: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if 'error' in tokens:
            return RedirectResponse(url=f"{os.environ.get('FRONTEND_URL', 'https://coinhubx-ui-refresh.preview.emergentagent.com')}/register?error=google_auth_failed")
        ...
        if existing_user:
            ...
            return RedirectResponse(url=f"{os.environ.get('FRONTEND_URL', 'https://coinhubx-ui-refresh.preview.emergentagent.com')}/auth/verify-phone?google_data={user_data_encoded}")
...
async def complete_google_signup(request: dict):
    ...
    <head>
        <title>Email Verified - Coin Hub X</title>
        <meta http-equiv="refresh" content="3;url=https://coinhubx-ui-refresh.preview.emergentagent.com/login">
        <style>
            body { 
                font-family: Arial, sans-serif; 
                background: linear-gradient(135deg, #000D1A, #1a1f3a);
    ...
    <body>
        <div class="container">
            ...
            <a href="https://coinhubx-ui-refresh.preview.emergentagent.com/login">Go to Login</a>
        </div>
    </body>
    </html>
    """
...
async def forgot_password(request: ForgotPasswordRequest, req: Request):
    ...
    try:
        ...
        reset_link = f"https://coinhubx-ui-refresh.preview.emergentagent.com/reset-password?token={reset_token}"
                    ...
                    html_content=f'''
                        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
                            <div style="background: linear-gradient(135deg, #000D1A, #1a1f3a); padding: 40px; text-align: center; border-radius: 10px;">
                                ...
                                <a href="https://coinhubx-ui-refresh.preview.emergentagent.com/dashboard" style="display: inline-block; background: linear-gradient(135deg, #00F0FF, #A855F7); color: #000; padding: 15px 40px; text-decoration: none; border-radius: 8px; font-weight: bold; margin-top: 30px;">
                                    View on Platform
                                </a>
                            </div>
                        </div>
...
async def initiate_withdrawal(request: InitiateWithdrawalRequest, req: Request):
    ...
    if security_level == "admin_approval_required":
        ...
        try:
            ...
            confirmation_url = f"{os.environ.get('FRONTEND_URL', 'https://coinhubx-ui-refresh.preview.emergentagent.com')}/confirm-withdrawal?token={confirmation_token}"
